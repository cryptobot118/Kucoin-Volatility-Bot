...
SIGNAL_NAME = 'djcommie_signalsell_rsi_stoch'
...
def analyze(pairs):
    ...
    for pair in pairs:
        ...
        if FULL_LOG:
            print(f'{SIGNAL_NAME}:{pair} Oscillators:{oscCheck}/{len(OSC_INDICATORS)} Moving averages:{maCheck}/{len(MA_INDICATORS)}, trading action:{action}')
